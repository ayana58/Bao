...
def add_fuzzy_features(data):
    ...
    # 调用模糊推理系统，生成推荐值
    recommendations = []
    #best_params = optimize_membership_function()
    #watch_time['medium'] = fuzz.trimf(watch_time.universe, best_params)
    for index, row in data.iterrows():
        recommendation_system.input['watch_time'] = row['watch_membership']
        recommendation_system.input['interest_level'] = row['interest_membership']
        #print(f"Watch time: {row['watch_membership']}, Interest level: {row['interest_membership']}")
        try:
            recommendation_system.compute()
...
if __name__ == "__main__":
    # 读取训练数据集的前100000行数据，每次读取1000行，以块的形式逐步处理
    data = pd.read_csv('./input/final_track1_train.txt', sep='\t', names=[
        'uid', 'user_city', 'item_id', 'author_id', 'item_city', 'channel', 'finish', 'like', 'music_id', 'did', 'creat_time', 'video_duration'], iterator=True, nrows=100000)
    ...
    dense_features.extend(['watch_membership', 'interest_membership', 'fuzzy_recommendation'])
